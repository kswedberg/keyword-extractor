...
var supported_languages = ["chinese", "danish","dutch","english","french","galician","german","italian","polish","portuguese","romanian","russian","spanish","swedish"];
var stopwords = require("./stopwords/stopwords");
var extractChinese = require("./extract_chinese");

const cleanString = (str) =>_.str.trim(_.str.stripTags(str));
...
function _extract(str, options){
    ...
    var text = Array.isArray(str) ? str.map((s) => cleanString(s)) : cleanString(str).split(/\s+/);
    if(_.isEmpty(text)){
        return [];
    }else{
        var words = _language === 'chinese' ? extractChinese(text) : text;
        var unchanged_words = [];
        var low_words = [];
        ...
        for(var x = 0;x < words.length; x++){
            ...
            if (_language === "chinese" && w.match(/[\s\u3000-\u303F\uFF00-\uFFEF]/g)) {
                w = "";
            }
            if(w.length > 0){
